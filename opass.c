...
void get_cli_args(int argc, char **argv)
{
	...
	int c = 0;
  	...
  	while ((c = getopt (argc, argv, "ehqvw:s:")) != -1) 
  	{
		...
		default:
			/* invalid option */
			fprintf(stderr,"\nWARNING: '%s': option '-%c' is invalid: ignored\n"
				,argv[0], optopt);
			break;
		}
	}
 ...
 * recieved via a pointer to this function. Returns a pointer to a new
...
char *with_spaces(char *newpass)
{
	...
	if (s_newpass == NULL) {
		fprintf(stderr,"Error allocating memory in function 'with_spaces()': %s\n",strerror(errno));
		exit(EXIT_FAILURE); 
	}
	...
	*s_newpass = '\0';
	...
	/* while we can read the *newpass string passed to this
	 * function */
	while ( *(newpass+n) != '\0' ) {
...
void show_help()
{
	...
	printf(""
	       ...
	       "  -w ?  Words   - specifiy how many three letter words make up the password (? == digits)\n"
	       "  -s ?  Suggest - specify how different passwords to offer (? == digits)\n");
}


/*
 * dump out the complete list of words held in our words array or
 * three letter words, that is used to generate passwords
 */
void dump_words()
{
	int i = 0;
...
char *get_random(int wordsRequired)
{
	...
	if (genPass == NULL) {
		fprintf(stderr,"Error allocating memory in function 'get_random()': %s\n",
			strerror(errno));
		exit(EXIT_FAILURE); 
	}
